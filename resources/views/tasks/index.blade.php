                ...
                <table class="table table-striped align-middle">
                    ...
                    @foreach ($tasks as $task)
                        <tr>                                
                            ...
                            <td class="{{ $task->is_urgent ? 'bg-red-100 text-red-700 font-bold' : '' }}">
                                {{ $task->title }}
                            </td>
                            
                            <td class="{{ $task->is_urgent ? 'bg-red-100' : '' }}">
                                {{ $task->description }}
                            </td>
                            ...
                            <td style="white-space: nowrap; width: 1px;" class="{{ $task->is_urgent ? 'bg-red-100' : '' }}">
                            ...
                            <td class="px-6 py-4 flex items-center gap-2 whitespace-nowrap {{ $task->is_urgent ? 'bg-red-100' : '' }}">
                                <form action="{{ route('tasks.complete', $task) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    @if($task->is_completed)
                                        <button type="submit" class="bg-gray-400 hover:bg-gray-500 text-white font-bold py-3 px-3 rounded text-sm">戻す</button>
                                    @else
                                        <button type="submit" class="bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-3 rounded text-sm">完了！</button>
                                    @endif
                                </form>
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap text-sm font-medium {{ $task->is_urgent ? 'bg-red-100' : '' }}">
